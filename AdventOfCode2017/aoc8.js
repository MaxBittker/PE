let program = `js inc 257 if wn < 9
jq dec -586 if esb != -3
gcf inc -603 if i >= -9
gcf dec -300 if d != 1
g inc -973 if gy > -1
...
function parseLine(line) {
  let [reg, direction, mag, _, x, cond, y] = line.split(" ");

  if (eval(`${get(x)} ${cond} ${y}`)) {
    let v = (direction === "inc" ? 1 : -1) * parseInt(mag, 10);
    registers[reg] = get(reg) + v;
    max = Math.max(max, registers[reg]);
  }
}
program.forEach(parseLine);
console.log(Object.keys(registers).map(get).reduce((a, b) => Math.max(a, b)));
console.log(max);
