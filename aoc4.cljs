...
(def passwords
      (map
        (fn [p] (vec (.split (.trim p) " ")))
        (.split input "\n")))

(defn process-word [word]
    (apply str (sort (vec word))))

(defn process-phrase [phrase]
   (set (map process-word phrase)))

(defn check-phrase [phrase]
    (=
        (count phrase)
        (count (process-phrase phrase))))

(defn count-list [pws]
    (count (filter check-phrase pws)))

(println passwords)

(println (count-list passwords))
