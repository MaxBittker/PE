...
(def program-data 
    (apply hash-map (apply concat (map (fn [[n c w]] [n [n c w]]) programs))))

; (defn adjustment [])   
(defn real-weight [name]
    (let [
          [name children weight] (program-data name) 
          child-weights 
            (map real-weight children)]
        (cond (not (apply = child-weights))
         (println name (map vector children child-weights)))      
        (+ (apply + child-weights) weight)))

(let [child-set (set (apply concat (map second programs)))
      program-names (map first programs)
      root (first (filter (fn [n] (not (child-set n))) program-names))]
    (println (real-weight root)))
