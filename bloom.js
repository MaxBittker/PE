...
bloomFilter.prototype = {
    hash: function(value, m) {
        ...
        return (hash * this.primes[m] % this.filter.length)
    },
...
for (var test = 0; test < count; test++) {
    ...
    if (bf.check(valuestr))
        error++ //it's not really there
        bf.add(valuestr)
    if (!bf.check(valuestr))
        console.log('false negative')

}
// console.log(bf.primes.length)
